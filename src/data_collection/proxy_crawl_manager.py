...
def build_proxies(ports: List[int]) -> Set[Optional[Dict[str, str]]]:
...
def start_collect_archive_proximity_set_indexes(proxies: Set[Optional[Dict[str, str]]]) -> None:
    ...
    with Pool(len(proxies)) as pool:
        pool.starmap(crawl_web_archive_cdx_worker, zip(partition_jobs(TIMESTAMPS, len(proxies)), proxies))
