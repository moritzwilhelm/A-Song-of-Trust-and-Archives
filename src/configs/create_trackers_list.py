...
from configs.utils import PROJECT_ROOT, INSTALLED_PROJECT_ROOT


def create_trackers_json() -> None:
    """Parse Disconnect list and create tracking.json containing the set of tracking domains."""
    with open(INSTALLED_PROJECT_ROOT.joinpath('configs', 'disconnect_entities.json')) as file:
    ...
    with open(PROJECT_ROOT.joinpath('src', 'configs', 'trackers.json'), 'w') as file:
